 ...
 */
async function createSearch({ fileds, partName, enum_connect, router_path }) {
  ...
  const typeName =
    ...
    partName
      ...
      .join("");
  const result = await compileEjs("part/config/search.config.ts.ejs", {
    fileds,
    typeName,
  });
  ...
  await writeFile(`${dest}/search.config.ts`, result);
  console.log("创建组件成功", `search.config.ts`);
}
